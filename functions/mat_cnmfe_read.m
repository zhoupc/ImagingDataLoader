function Y = mat_cnmfe_read(mat_file, t_range, r_range, c_range)
...
% determine the smallest block that includes the patch
r0_patch = r_range(1);
r1_patch = r_range(2);
c0_patch = c_range(1);
c1_patch = c_range(2);
...
for m=1:nr_block
    ...
    for n=1:nc_block
        ...
        nc = c1-c0+1;
        if isOnWorker
            Y((r0:r1)-block_rstart, (c0:c1)-block_cstart, :) =  eval(sprintf('mat_data.Y_%d_%d_%d_%d(1:nr, 1:nc, %d:%d);', r0, r1, c0, c1, t_range(1), t_range(2)));
        else
            tmp_str = sprintf('%s.Y_%d_%d_%d_%d(1:%d, 1:%d, %d:%d);', data_nam, r0, r1, c0, c1, nr, nc, t_range(1), t_range(2));
